...
class AdminAsetController extends Controller
{
    public function index(){
        ...
        return view('pengaturan.adminaset',[
            'title' => 'Pengaturan',
            'page' => 'Admin Aset',
            'drops' => $instansi
        ]);
    ...
    public function data(){
        $admin = User::where('level','=','aset')->get();
        return datatables()->of($admin)
                ...
                ->addColumn('aksi', function($admin){
                    return '
                            <div class="btn-group">
                                <a href="javascript:void(0)" onclick="editAdmin(`'.route('adminaset.update',$admin->id).'`,'.$admin->id.')" class="btn btn-warning" ><i class="fas fa-edit"></i></a>
                                <a href="javascript:void(0)" onclick="hapusAdmin(`'.route('adminaset.destroy',$admin->id).'`)" class="btn btn-danger" ><i class="fas fa-trash"></i></i></a>
                            </div>
                            ';
                })
                ->rawColumns(['aksi'])
    ...
    public function store(Request $request){
        ...
        User::create($data);
        return response()->json('user aset berhasil ditambahkan',200);
    }
    ...
    public function update(Request $request,$id){
        ...
        $admin->update($data);
        return response()->json('user aset berhasil diubah',200);
    }

    public function destroy($id){
        $admin = User::find($id);
        $admin->delete();
        return response('user aset berhasil dihapus', 204);
    }
}
