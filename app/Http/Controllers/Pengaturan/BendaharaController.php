...
class BendaharaController extends Controller
{
    ...
    public function data(){
        ...
        return datatables()->of($admin)
                ...
                ->addColumn('aksi', function($admin){
                    return '
                            <div class="btn-group">
                                <a href="javascript:void(0)" onclick="editAdmin(`'.route('bendahara.update',$admin->id).'`,'.$admin->id.')" class="btn btn-warning" ><i class="fas fa-edit"></i></a>
                                <a href="javascript:void(0)" onclick="hapusAdmin(`'.route('bendahara.destroy',$admin->id).'`)" class="btn btn-danger" ><i class="fas fa-trash"></i></i></a>
                            </div>
                            ';
                })
                ->rawColumns(['aksi'])
    ...
    public function store(Request $request){
        ...
        User::create($data);
        return response()->json('user bendahara berhasil ditambahkan',200);
    }
    ...
    public function update(Request $request,$id){
        ...
        $admin->update($data);
        return response()->json('user bendahara berhasil diubah',200);
    }

    public function destroy($id){
        $admin = User::find($id);
        $admin->delete();
        return response('user bendahara berhasil dihapus', 204);
    }
}
