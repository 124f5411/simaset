        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">
                    ...
                    <a href="#" onclick="pilihTahun('{{ route('sbu.exportForm') }}')" class="btn btn-sm btn-danger btn-icon-split mt-2 float-right">
                ...
                <div id="results" class="row">
                    @foreach ($drops['instansi'] as $value)
                        <div id="result" class="col-xl-6 col-md-6 mb-4">
                            <a href="{{ route('sbu.instansi',encrypt($value->id)) }}" class="btn btn-block btn-outline-primary">
            ...
            $('#modalExport').validator().on('submit', function (e){
                if(! e.preventDefault()){
                    ...
                    .done((response) => {
                        ...
                        window.open("{{route('sbu.exportAset', ['',''])}}"+"/"+response.tahun+"/"+response.jenis,'Title','directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=1024,height = 720');
                        setTimeout(function(){
