    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                ...
                <h6 class="m-0 font-weight-bold text-primary">{{ getValue("opd"," data_opd", " id = ".decrypt(Request::segment(3))) }}</h6>
                <a href="#" onclick="addForm('{{ route('tanah.store',Request::segment(3)) }}')" class="btn btn-primary btn-icon-split float-right">
...
<script>
    ...
    function addForm(url) {
        $('#form-tanah').slideToggle('fast', function () {
            ...
            $('#id_jenis').change(function() {
                ...
                let kode = '{{ $kode_tanah }}';
                let sendKode = kode+'.'+id_jenis;
                if(id_jenis === ""){
                    $('#kode').val('');
                    $('#register').val('');
                }else{
                    $('#kode').val(sendKode);
                    let getRegis = "{{route('tanah.getRegister', ['',''])}}"+"/"+sendKode+"/"+id_opd;;
                    $.get(getRegis)
                    .done((response) => {
    ...
    function editTanah(url,id) {
        ...
        $('#frm-tanah').attr('action', url);
            ...
            .done((response) => {
                ...
                $('#frm-tanah [name=id_jenis]').val(response.id_jenis).trigger('change').select2({disabled:'readonly'});
    ...
    function verifTanah(url){
        if (confirm('yakin kirim data ke admin aset terpilih?')) {
                $.post(url, {
                        '_token': $('[name=csrf-token]').attr('content'),
                        '_method': 'put'
                    })
